...
import { getTaskList, addTaskList, updateTaskList, deleteTaskList } from "../../services/backendApiHelper";
...
const TaskList = () => {
  ...
  const [modal, setModal] = useState<ModalState>({ isOpen: false, type: "", task: null });
  ...
  const handleSave = async () => {
    try {
      ...
      fetchTasks();
      setModal({ isOpen: false, type: "", task: null });
    } catch (error) {
  ...
  return (
    <div className="p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {tasks.map((task) => (
          <div key={task.id} className="border border-gray-300 rounded-lg p-4 shadow-md">
